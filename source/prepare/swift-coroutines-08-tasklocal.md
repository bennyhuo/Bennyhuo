...
TaskLocal 就是 Task 私有的变量，不同的 Task 对于这个变量的访问将得到不同的结果。

下面我们给出示例演示如何定义一个 TaskLocal 变量：
...
TaskLocal 变量必须定义为静态的存储属性，并使用 TaskLocal 这个属性包装器（property wrapper）来包装。TaskLocal 变量也受限于属性包装器的支持范围，不能定义为顶级属性。
...
TaskLocal 变量虽然看起来就是个静态存储属性，但它的值实际上是存储在 Task 相关的内存当中的。它的读写性能自然也与它的存储方式有关，因此为了确保能够正确合理的使用 TaskLocal，我们有必要了解一下它究竟是如何存储的。
...
显然，绝大多数情况下 TaskLocal 变量的数量都不会很多，同时插入的值只在 withValue 函数范围内有效也使得绝大多数查找的值都排在链表前面，因此线性查找的效率并不会存在性能问题。
...
简单总结一下，TaskLocal 变量是存在链表当中的，我们在使用过程中应当避免使用过多的 TaskLocal 变量，也应该适当地减少对 TaskLocal 变量的访问次数，以避免性能上最坏的情况出现。
